...
export default function Header() {
  const [search, setSeatch] = React.useState<string | null>(null);
    ...
    <header className="h-32">
      <div className="flex items-center h-full justify-between w-full px-24">
        ...
        <div className="relative w-1/2">
          <input type="text" placeholder="O que está buscando hoje?" className="w-full h-12 bg-white rounded-full px-4 pr-12 placeholder:text-gray-400 focus:outline-none" onChange={(e) => setSeatch(e.target.value)} />
          <button onClick={searchProduct} className="absolute right-8 top-1/2 transform -translate-y-1/2 text-gray-600 hover:text-gray-600 transition-colors cursor-pointer">
        ...
        <div className="flex items-center gap-8">
          {user ? (
            <div className="flex items-center gap-4">
              ...
              <button onClick={handleLogout} className="flex items-center gap-2 px-3 py-2 text-sm text-red-600 hover:bg-red-50 rounded-md transition-colors">
