...
export class AccountRepository {
  ...
  async update(data: UpdateUserDto, id: number) {
    return await this.prisma.user.update({ data: { ...data, birthdate: new Date(data.birthdate) }, where: { id } });
  }
  ...
  async getUserStats(userId: number) {
    const [ordersCount, wishlistItemsCount, addressesCount, totalSpent] = await Promise.all([
  ...
  async getUserActivities(userId: number) {
    const [recentOrders, recentWishlistItems, recentAddresses] = await Promise.all([
    ...
    recentOrders.forEach((order) => {
      ...
      const statusText = status === "COMPLETED" ? "entregue" : status === "SHIPPED" ? "enviado" : status === "APPROVED" ? "aprovado" : "pendente";
