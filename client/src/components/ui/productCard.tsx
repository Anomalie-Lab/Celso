...
export default function ProductCard({ data }: { data: Product }) {
  ...
  return (
    <div className="bg-white rounded-lg p-4 relative w-ful transition-shadow cursor-pointer">
      {/* Selo de desconto */}
      <div className="absolute top-10 left-10 bg-primary-100 text-primary-500 text-[10px] font-bold px-2 py-1 rounded-full z-10">{data.discount}</div>
      ...
      <div className="relative mb-4">
        <div className="w-full h-68 bg-gray-100 rounded-lg flex items-center justify-center relative overflow-hidden" style={{ backgroundImage: `url(${data.image})`, backgroundSize: "cover", backgroundPosition: "center" }}></div>
      </div>
      ...
      {/* Título do produto */}
      <h3 className="text-gray-800 font-semibold text-md mb-2 leading-tight">{data.productName}</h3>

      {/* Avaliação */}
      <div className="flex items-center gap-2 mb-2">
        <div className="flex items-center">{renderStars(data.rating)}</div>
        <span className="text-gray-500 text-sm">
          {data.rating.toFixed(1)} ({data.reviews} Avaliações)
        </span>
      </div>
      ...
      <div className="space-y-1">
        <div className="flex items-center gap-2">
          <span className="text-gray-800 font-bold text-xl">{data.currentPrice}</span>
          <span className="text-gray-400 text-sm line-through">{data.originalPrice}</span>
        </div>
        <p className="text-gray-500 text-sm">ou 1x de {data.currentPrice}</p>
      </div>
    </div>
  );
}
