...
export const LoginUser = (res: Response, user: Account.UserI) => {
  ...
  res.cookie(process.env.NEXT_SESSION_COOKIE, token, {
    httpOnly: true,
    secure: process.env.NEXT_NODE_ENV === "production",
    sameSite: "lax",
    domain: process.env.NEXT_NODE_ENV === "production" ? ".daeshis.site" : undefined,
    maxAge: 7 * 24 * 60 * 60 * 1000,
  });

  res.cookie(process.env.NEXT_IS_IN_SESSION, "true", {
    httpOnly: false,
    secure: process.env.NEXT_NODE_ENV === "production",
    sameSite: "lax",
    domain: process.env.NEXT_NODE_ENV === "production" ? ".daeshis.site" : undefined,
...
export const LogoutUser = (res: Response) => {
  res
    .cookie(process.env.NEXT_SESSION_COOKIE, "", {
      httpOnly: false,
      secure: process.env.NEXT_NODE_ENV === "production",
      sameSite: "lax",
      domain: process.env.NEXT_NODE_ENV === "production" ? ".daeshis.site" : undefined,
    ...
    .cookie(process.env.NEXT_IS_IN_SESSION, "", {
      httpOnly: false,
      secure: process.env.NEXT_NODE_ENV === "production",
      sameSite: "lax",
      domain: process.env.NEXT_NODE_ENV === "production" ? ".daeshis.site" : undefined,
...
export const GetSessionToken = (res: Response, req: Request) => {
  const token = req.cookies[process.env.NEXT_SESSION_COOKIE];
