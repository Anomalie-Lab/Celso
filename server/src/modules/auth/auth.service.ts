...
export class AuthService {
  ...
  async register(dto: RegisterDto) {
    try {
      ...
      const passwordHash = await bcrypt.hash(dto.password, salt);
        ...
        const token = sign({ username: dto.username, password: dto.password }, process.env.NEXT_SECRET);
  ...
  async login(dto: LoginDto, res: ExpressResponse) {
    try {
      ...
      if (!isValidPassword) throw new HttpException("Senha incorreta.", HttpStatus.UNAUTHORIZED);
      LoginUser(res, user);
      return user;
    } catch (error) {
      HandleErrorsUserConflict(error);
  ...
  async google(res: ExpressResponse, req: ExpressRequest) {
    ...
    try {
      ...
      return res.status(HttpStatus.OK).redirect(process.env.NEXT_APP_URL);
    } catch (error: any) {
      HandleErrorsUserConflict(error);
    }
  }
  ...
  async forgotPassword(res: ExpressResponse, email: string) {
    try {
      ...
      if (!user) throw new HttpException("Conta não encontrada.", HttpStatus.NOT_FOUND);

      const token = await this.authRepository.createPasswordReset(email, user.id);
  ...
  async resetPassword(res: ExpressResponse, dto: ResetPasswordUserDto) {
    try {
      const { email, id } = verify(dto.token, process.env.NEXT_SECRET) as { email: string; id: number };

      if (email !== dto.email) throw new HttpException("The emails do not match.", HttpStatus.NOT_ACCEPTABLE);
      ...
      throw new HttpException("The link is expired or invalid.", HttpStatus.NOT_ACCEPTABLE);
    }
  }
}
